...
class TestOperationIdSearch:
    ...
    async def test_natural_language_search_for_revoke(self, searcher):
        ...
        for query in queries:
            ...
            assert revoke_found, f"Query '{query}' should find auth_token_revoke_api in top 5 results, but found: {[r.get('operation_id') for r in results[:5]]}"
        ...
        assert results[0].get("operation_id") == "auth_token_revoke_api", f"Query 'revoke' should return auth_token_revoke_api as top result, got: {results[0].get('operation_id')}"
    ...
    async def test_revoke_token_includes_both_revoke_apis(self, searcher):
        ...
        assert "auth_token_revoke_api" in operation_ids, f"auth_token_revoke_api should be in revoke token search results. Found: {operation_ids[:5]}"
        assert "auth_revocation_api" in operation_ids, f"auth_revocation_api should be in revoke token search results. Found: {operation_ids[:5]}"
        ...
        print(f"Search results positions:")
        ...
        assert revoke_api_position <= 10, f"auth_token_revoke_api should be in top 10 results, found at position {revoke_api_position}"
        assert revocation_api_position <= 10, f"auth_revocation_api should be in top 10 results, found at position {revocation_api_position}"
